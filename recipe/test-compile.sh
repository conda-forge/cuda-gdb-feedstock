...
cuda-gdb -ex "show style sources" -batch | grep "enabled"
...
if [[ $(uname) == "Darwin" && $(uname -m) == "x86_64" ]]; then
  ...
  $CC -o hello -g "./testing/hello.c"
else
  # TODO: Remove mold usage once https://github.com/conda-forge/binutils-feedstock/issues/97
  # is done.
  $CC -o hello -g -gz=zstd -fuse-ld=mold "./testing/hello.c"
fi
cuda-gdb -batch -ex "run" --args hello
...
cuda-gdb -batch -ex "run" -ex "py-bt" --args python "./testing/process_to_debug.py" | tee gdb_output
